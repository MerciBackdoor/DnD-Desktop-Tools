...
function createWindow(app) {
  ...
  // Применяем дефолтные размеры из app-listeners.js (только для новых окон)
  if (typeof applyWindowDefaults === 'function') applyWindowDefaults(app);
...
// ── Обработчики межоконных событий вынесены в app-listeners.js ───────────────
  ...
  try {
    ...
    if (session.openWindowsData && session.openWindowsData.length > 0) {
      session.openWindowsData.forEach(wData => {
        ...
        app._fromSession = true; // защита от перезаписи дефолтами
